...
const RESEND_API_KEY = process.env.EXPO_PUBLIC_RESEND_API_KEY || 're_demo_key';

export const passwordResetService = {
  // Demander un code de réinitialisation
  requestResetCode: async (email) => {
    try {
      console.log('📧 [passwordResetService] Demande code pour:', email);
      ...
      if (codeError) {
        console.error('❌ [passwordResetService] Erreur génération code:', codeError);
        throw codeError;
      }
      
      console.log('✅ [passwordResetService] Code généré:', codeData.code);
      
      // 2. Envoyer l'email via Resend API
      try {
        console.log('📧 [passwordResetService] Envoi email via Resend...');
        ...
        if (!response.ok) {
          console.error('❌ [passwordResetService] Erreur Resend:', result);
          throw new Error(result.message || 'Erreur envoi email');
        }
        
        console.log('✅ [passwordResetService] Email envoyé avec succès via Resend');
      } catch (emailError) {
        console.error('❌ [passwordResetService] Erreur envoi email:', emailError);
      ...
      console.error('❌ [passwordResetService] Erreur:', error);
      throw error;
    }
  },
  ...
  resetPasswordWithCode: async (email, code, newPassword) => {
    try {
      console.log('🔑 [passwordResetService] Reset pour:', email);
      ...
      console.log('✅ [passwordResetService] Mot de passe changé');
      return data;
    } catch (error) {
      console.error('❌ [passwordResetService] Erreur:', error);
      throw error;
